...
#
# ===========================================================
# Pololu 3pi+ 2040 OLED — Local Greedy Search (UART → ESP32 → MQTT)
# ===========================================================
...
#
# Behavior overview (Algorithm C):
#   * Local greedy goal selection within Manhattan distance window (2 steps).
#   * Pre-clue: forward bias with side check secondary (serpentine behavior).
#   * Post-clue: pure probability, no directional preference.
#   * A* planning with turn costs (halved post-clue), visited penalty, centerward cost.
...
#
# Threads:
#   * A background movement thread handles forward motion while main thread processes UART.
...
METRICS_LOG_FILE = "metrics-log-00C.txt"
...
intersection_visits = {}
unique_cells_count = 0       # cells first visited by this robot on behalf of the team
...
def handle_msg(line):
    ...
    """
    global peer_intent, peer_pos, first_clue_seen, object_location, start_signal, found_object, FIRST_CLUE_TIME_MS
...
def update_prob_map():
    ...
    - Post-clue: sum overlapping rewards from all clues (1.0/(1+Manhattan))
    - Visited cells get zero probability (matches B-sim.py)
    """
    ...
    for y in range(GRID_SIZE):
        for x in range(GRID_SIZE):
            ...
            if len(clues) == 0:
                # Pre-clue: uniform over unvisited cells
                prob_map[i] = 1.0 / (GRID_SIZE * GRID_SIZE)
            else:
                # Post-clue: sum overlapping rewards from all known clues
                clue_sum = 0.0
                for (cx, cy) in clues:
                    clue_sum += 1.0 / (1 + abs(x - cx) + abs(y - cy))
                prob_map[i] = clue_sum
...
def pick_goal():
    ...
    Returns goal cell or None if no goals available.
    """
    # Simple energy tracking - no function call counting needed
    max_steps = GOAL_VISIBILITY_STEPS if GOAL_VISIBILITY_STEPS >= 0 else 0
    best = None
    best_val = -1e9

    def within_window(cell):
        return abs(cell[0] - pos[0]) + abs(cell[1] - pos[1]) <= max_steps
    ...
    if best is None and heading != (0, 0):
        ...
        for sx, sy in (left, right):
            ...
            if not (0 <= nx < GRID_SIZE and 0 <= ny < GRID_SIZE):
                continue
            side_cell = (nx, ny)
            if not within_window(side_cell):
                continue
            i = idx(nx, ny)
            if grid[i] != CELL_UNSEARCHED:
                continue
    ...
    for cell in window_cells:
        ...
        if not first_clue_seen and abs(cell[0] - pos[0]) + abs(cell[1] - pos[1]) != 1:
            continue
    ...
    if best is None:
        unknowns = [cell for cell in window_cells if grid[idx(cell[0], cell[1])] == CELL_UNSEARCHED]
        if unknowns:
            best = min(unknowns, key=lambda c: abs(c[0] - pos[0]) + abs(c[1] - pos[1]))
        else:
            revisit_candidates = [cell for cell in window_cells if cell != (pos[0], pos[1])]
            if not revisit_candidates:
                return None
            best = min(
                revisit_candidates,
                key=lambda c: (intersection_visits.get((c[0], c[1]), 0), abs(c[0] - pos[0]) + abs(c[1] - pos[1]))
            )
    return best
# ===========================================================
...
def a_star(start, goal):
    ...
    A* over the 4-neighbor grid, with costs:
      +1 per step
      + TURN_COST × quarter_turns (halved after first clue)
    ...
    while frontier and running and not found_object:
        ...
        for dx, dy in DIRS4:
            ...
            step_cost += turn_cost_per_turn * turns

            if grid[i] == CELL_SEARCHED:
                step_cost += cfg.VISITED_STEP_PENALTY

            step_cost += centerward_step_cost(cx, cy, nx, ny)

            blocked_by_peer = False
            for cell in peer_intent.values():
                if cell is None:
                    continue
                if cell == (nx, ny):
...
# ===========================================================
def search_loop():
    """Main mission loop (Algorithm C - Local Greedy).

    1. Choose a goal within Manhattan window (2 steps), forward/side biased pre-clue.
    2. Plan with A* using turn, center, intent, and reward costs.
    3. Publish intent, turn, and advance one cell (abort on bump).
    4. Mark the cell, report status, and check for clues.
    5. Repeat until the object is found or no goals remain.

    Motors are always stopped in a ``finally`` block.
    """
    global first_clue_seen, move_forward_flag, start_signal, METRIC_START_TIME_MS, pos, yield_count, path_replan_count, goal_replan_count, current_goal, FIRST_CLUE_TIME_MS, system_clues_found, FIRST_CLUE_POSITION, system_visits, busy_ms, mem_free_min
