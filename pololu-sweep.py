...
def generate_sweep_path(active_ids):
    ...
    if total == 4:
        ...
        def owner(x, y):
            s = x + y
            if s < GRID_SIZE - 1:
                return "00" if x >= y else "02"
            elif s > GRID_SIZE - 1:
                return "03" if x > y else "01"
            else:
                if x > y:
                    return "03"
                elif x < y:
...
def handle_msg(line):
    ...
    """
    global peer_intent, peer_pos, object_location, start_signal, found_object, system_visits, system_repeat_count, FIRST_CLUE_TIME_MS, OBJECT_TIME_MS, OBJECT_STEP_COUNT
