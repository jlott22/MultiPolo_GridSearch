...
#
"""
Pololu Search Simulator — E Algorithm (Pre-Clue Behavior Only, No Clue Reactions)

E Algorithm - Control Implementation:
- Uses A*/B*/C* pre-clue behavior (centerward bias, probability maps, A* planning)
- NEVER reacts to clues (always behaves as if no clues have been found)
- Minimal messaging like D algorithm (no intent/goal messages)
- Useful for isolating the impact of clue-driven behavior changes

Key behaviors:
- Async scheduler: robots update on independent timelines with configurable jitter
- Team sizes: 1, 2, or 4 robots. 4 = four corners facing inward; 2 = opposite corners facing inward
- Goal selection: argmax(prob_map * REWARD_FACTOR) over unknown cells (no peer coordination)
- Prob map: ALWAYS uniform (clues detected but never update probability map)
- A* on 4-connected grid with costs:
      MOVE_COST + TURN_COST * quarter_turns +
      centerward_step_cost (ALWAYS applied - never disabled) +
      VISITED_STEP_PENALTY (onto visited)
- Centerward penalty: ALWAYS applied (never disabled, even after finding clues)
- Messaging: Minimal like D algorithm (position, visited, clue, object only - no intent/goal)
...
Usage examples:
  python3 E-sim-async.py --mode batch --episodes 200 --robots 4 --grid 10 --csv out.csv
  python3 E-sim-async.py --mode view --robots 4 --grid 10 --viewer-fps 1 --show-truth
...
#  6. CSV results auto-save to: sim_results_E1.csv (in current directory)
...
DEFAULT_REWARD_FACTOR = 8.0
DEFAULT_CENTER_STEP = 0.4
DEFAULT_VISITED_STEP_PENALTY = 2.5
...
class Config:
    ...
    csv_path: str = 'sim_results_E1.csv'
    viewer_fps: int = 4
    cell_px: int = 10
...
class Robot:
        ...
        for y in range(size):
            for x in range(size):
                ...
                if cell in self.know.visited:
                    ...
                    continue
                # E Algorithm: ALWAYS use uniform probability (never update based on clues)
                prob_map[self._idx(x, y)] = 1.0 / total_cells
        return prob_map

    def centerward_step_cost(self, curr: Cell, nxt: Cell) -> float:
        # E Algorithm: ALWAYS apply centerward bias (never disable it)
        size = self.cfg.grid_size
        c = (size - 1) / 2.0
        cost = 0.0
    ...
    def plan_path(self, goal: Cell,
        ...
        # E Algorithm: ALWAYS use full turn cost (pre-clue behavior)
        turn_cost_per_turn = TURN_COST
    ...
    def step_once(self, reserved_goals: Dict[str, Cell],
        ...
        plan_peer_positions: Dict[str, Cell]
        plan_peer_intents: Dict[str, Optional[Cell]]
        if self.comm:
            plan_peer_positions = self.comm.snapshot_peer_positions()
            plan_peer_intents = self.comm.snapshot_peer_intents()
        else:
            plan_peer_positions = {rid: pos for rid, pos in peer_positions.items() if rid != self.rid}
            plan_peer_intents = {rid: intent for rid, intent in peer_intents.items() if rid != self.rid}
        ...
        if goal_changed:
            ...
            forced_by_peer = (self.goal_replan_pending and not self.goal_completed_by_self_pending)
            # Count goal replans only when another robot invalidated our previous goal
            if (self.know.first_clue_seen and previous_goal is not None and forced_by_peer
                    and not self.path_replan_pending):
                self.goal_replans += 1
            self.goal_replan_pending = False
            self.goal_completed_by_self_pending = False
            # E Algorithm: No goal messaging (like D algorithm)
    ...
    def _execute_pending_action(
        ...
        if target is not None:
            ...
            # E Algorithm: No intent messaging (like D algorithm)
        ...
        if kind == "move":
            ...
            blocked = False
            if self.comm:
                # Use delayed snapshot from comm system
                perceived_intents = self.comm.snapshot_peer_intents()
                perceived_positions = self.comm.snapshot_peer_positions()
                ...
                # Fallback to ground-truth if no comm (shouldn't happen in async mode)
                for pid, pint in peer_intents.items():
...
def collect_episode_result(world: World, know: Knowledge, robots: List['Robot'], steps_total: int, found: bool) -> EpisodeResult:
    ...
    # Determine steps before/after the first clue, if any clue was seen
    first_clue_step = getattr(know, "steps_before_first_clue", None)
    if getattr(know, "first_clue_seen", False) and isinstance(first_clue_step, int) and first_clue_step >= 0:
        steps_before_first_clue = first_clue_step
        steps_after_first_clue = max(0, steps_total - first_clue_step)
    else:
        steps_before_first_clue = steps_total
        steps_after_first_clue = 0

    robot_positions_at_first_clue = getattr(know, "robot_positions_at_first_clue", {}) or {}
    ...
    return EpisodeResult(
        ...
        robot_positions_at_first_clue=robot_positions_at_first_clue,
...
def parse_args(argv=None) -> Config:
    ...
    p.add_argument("--async-step-mean", type=float, default=base_cfg.async_step_mean,
                   help="Mean wall-clock seconds between robot updates in the async scheduler (default: 1.85).")
    ...
    p.add_argument("--async-min-delay", type=float, default=base_cfg.async_min_delay,
                   help="Minimum time between updates for any robot (default: 1.75).")
    p.add_argument("--async-max-delay", type=float, default=base_cfg.async_max_delay,
                   help="Maximum time between updates for any robot (default: 1.95; set <=0 to disable clamp).")
    ...
    p.add_argument("--async-turn-quarter", type=float, default=base_cfg.async_turn_quarter,
                   help="Seconds required for a single 90-degree turn (default: 3.0).")
