...
#
"""
Pololu Search Simulator (v3 async) -- C-00 Local Greedy Algorithm with asynchronous robot clocks,
message delays, and telemetry counters, fixed placements per episode, batch + viewer.

Key behaviors (C-00 specific + async enhancements):
- Async scheduler: robots update on independent timelines with configurable jitter so coordination uses slightly stale peer data.
- Default cadence models hardware: each move waits ~1.50-1.70 seconds between cells (nominal 1.60s, +/-0.1); turns cost ~0.3s per quarter turn.
- Team sizes: 1, 2, or 4 robots. 4 = four corners facing inward; 2 = opposite corners facing inward.
- Goal selection: Local greedy within Manhattan distance window (default 2 steps).
  Prefers forward/side cells first, then the highest-probability cell in the window. No goal reservation.
- Telemetry: message counters track communication load for later analysis.
...
- A* pathfinding on 4-connected grid with turn costs (halved post-clue), visited penalty, centerward cost.
- Simple collision avoidance: avoids peer positions and declared intents.
- Centerward bias is only applied until the FIRST clue is found by the team.
- Communication delay: messages between robots incur a configurable delay (default 150ms) simulating MQTT/network latency.
...
Usage examples:
  python3 C-sim-async.py --mode batch --episodes 200 --robots 4 --grid 10 --csv out.csv
  python3 C-sim-async.py --mode batch --robots 2 --clues 3 --episodes 500 --trials-out trials.json
  python3 C-sim-async.py --mode view --robots 4 --grid 10 --viewer-fps 1 --show-truth --trials-in trials.json
...
#  1. Run: py C-sim-async.py --mode view
...
#  6. CSV results auto-save to: sim_results_C_async.csv (in current directory)
...
# ------------------------
# Constants (C-00 defaults â€" overridable via CLI)
# ------------------------
MOVE_COST = 1.0
TURN_COST = 1.0
...
DEFAULT_GOAL_VISIBILITY_STEPS = 2
...
class Config:
    ...
    goal_visibility_steps: int = DEFAULT_GOAL_VISIBILITY_STEPS
    episodes: int = 4
    mode: str = "batch"  # "batch" or "view"
    csv_path: str = "sim_results_C1.csv"
    viewer_fps: int = 1
    cell_px: int = 10
    show_truth_in_viewer: bool = True
    trials_in: Optional[str] = None
    trials_out: Optional[str] = None
    scenarios_csv: Optional[str] = "trials_d2_c2_g10.csv"
    ...
    async_min_delay: float = 1.5
    async_max_delay: float = 1.7
    ...
    comm_delay_s: float = 0.05  # Communication delay between robots
...
def aggregate_team_knowledge(robots: List['Robot']) -> Knowledge:
    """Aggregate per-robot knowledge into a delayed team view for metrics/viewer."""
    agg = Knowledge()
    for rb in robots:
        # Merge visited counts from robot knowledge
        for cell, count in rb.know.visited.items():
            existing = agg.visited.get(cell, 0)
            if count > existing:
                agg.visited[cell] = count
        # Merge comm-observed system visits (captures delayed peer updates)
        if rb.comm:
            for cell, count in rb.comm.system_visits.items():
                existing = agg.visited.get(cell, 0)
                if count > existing:
                    agg.visited[cell] = count
        ...
        for cell in rb.know.known_clues:
            if cell not in agg.known_clues:
                agg.known_clues.append(cell)

        # Revisits metrics
        agg.individual_revisits += rb.know.individual_revisits
        agg.system_revisits += rb.know.system_revisits

        if rb.know.first_clue_seen:
            agg.first_clue_seen = True
            if rb.know.first_clue_step is not None:
                if agg.first_clue_step is None or rb.know.first_clue_step < agg.first_clue_step:
                    agg.first_clue_step = rb.know.first_clue_step
            for rid, cell in rb.know.robot_positions_at_first_clue.items():
                if rid not in agg.robot_positions_at_first_clue:
                    agg.robot_positions_at_first_clue[rid] = cell
    return agg

# ------------------------
# Communication (C-00 style with async delays)
# ------------------------
...
class CommModule:
    ...
    def publish_position(self, cell: Cell) -> None:
        ...
        self.robot.position_msgs_sent += 1
    ...
    def publish_visited(self, cell: Cell) -> None:
        ...
        self.robot.visited_msgs_sent += 1
    ...
    def receive(self, msg: CommMessage) -> None:
        ...
        if msg.topic == '1':
            ...
            self.robot.position_msgs_received += 1
        elif msg.topic == '2':
            cell = self._parse_cell(payload)
            if cell is None:
                return
            # Track system-wide revisits (matches A-00.py behavior)
            ...
            self.robot.visited_msgs_received += 1
            self.robot.ingest_peer_visit(cell)
        elif msg.topic == '3':
            cell = self._parse_cell(payload)
            if cell is None:
                return
            ...
            self.robot.clue_msgs_received += 1
            self.robot.ingest_peer_clue(cell)
        elif msg.topic == '4':
            cell = self._parse_cell(payload)
            if cell is None:
                return
            ...
            self.robot.object_msgs_received += 1
            self.robot.ingest_peer_object(cell)
        elif msg.topic == '5':
            cell = self._parse_cell(payload)
            if cell is None:
                return
            ...
            self.robot.intent_msgs_received += 1
        elif msg.topic == '6':
            ...
            self.robot.goal_msgs_received += 1
...
class Robot:
    ...
    # Message counters for lightweight telemetry
    ...
    goal_msgs_sent: int = 0
    goal_msgs_received: int = 0
    ...
    def sync_from_comm(self) -> None:
        """Incorporate any delayed peer updates delivered via the comm module."""
        if self.comm is None:
            return

        for cell, count in self.comm.system_visits.items():
            if count > self.know.visited.get(cell, 0):
        ...
        for cell in self.comm.received_clues:
            ...
            if not self.know.first_clue_seen:
                self.know.first_clue_seen = True
                if self.know.first_clue_step is None:
                    self.know.first_clue_step = self.steps_taken
                self.know.robot_positions_at_first_clue.setdefault(self.rid, self.pos)

    def _capture_perception(self) -> None:
        """Record peer perception for the next planning cycle."""
        if self.comm is None:
    ...
    def _promote_perception(self) -> Tuple[Dict[str, Cell], Dict[str, Optional[Cell]], Dict[str, Optional[Cell]]]:
        """Return the frozen peer state used when planning a new move."""
    ...
    def record_visit(self, cell: Cell, source: str = "self", count_visit: bool = True) -> None:
        if source == "self":
            if count_visit:
                # System revisits: any prior visit observed through delayed comm data
                if self.comm and self.comm.system_visits.get(cell, 0) > 0:
                    self.system_revisits += 1
                    self.know.system_revisits += 1

                if cell in self.know.visited:
                    self.know.individual_revisits += 1
                self.know.visited[cell] = self.know.visited.get(cell, 0) + 1
                if self.comm:
                    self.comm.system_visits[cell] = self.comm.system_visits.get(cell, 0) + 1
        else:
            # Peer visits: align local knowledge with comm-observed counts (applied on delayed delivery)
            if self.comm:
                observed = self.comm.system_visits.get(cell, 0)
                if observed > self.know.visited.get(cell, 0):
                    self.know.visited[cell] = observed

    def record_clue(self, cell: Cell, source: str = "self") -> None:
        if cell not in self.know.known_clues:
            self.know.known_clues.append(cell)
        if not self.know.first_clue_seen:
            self.know.first_clue_seen = True
    ...
    def ingest_peer_visit(self, cell: Cell) -> None:
        self.record_visit(cell, source="peer")

    def ingest_peer_clue(self, cell: Cell) -> None:
        self.record_clue(cell, source="peer")

    def ingest_peer_object(self, cell: Cell) -> None:
        # Object announcements are currently informational only.
        return
    ...
    def build_prob_map(self) -> List[float]:
        ...
        for y in range(size):
            for x in range(size):
                cell = (x, y)
                comm_visits = self.comm.system_visits if self.comm else {}
                if cell in self.know.visited or cell in comm_visits:
    ...
    def plan_path(self, goal: Cell,
        ...
        while frontier:
            ...
            for dx, dy in DIRS4:
                ...
                # Treat peer-reserved or occupied cells as obstacles to match firmware
                blocked_by_peer = any(
    ...
    def pick_goal(
        self,
        prob_map: List[float],
        reserved_goals: Optional[Dict[str, Optional[Cell]]] = None,
        peer_positions: Optional[Dict[str, Cell]] = None,
        peer_intents: Optional[Dict[str, Optional[Cell]]] = None,
    ) -> Tuple[Optional[Cell], float]:
        """Local greedy goal selection within Manhattan distance window.

        Pre-clue: forward bias, then sides (serpentine behavior)
        Post-clue: pure probability, no directional preference

        Additional parameters are accepted for interface compatibility but are not used.

        Returns (goal_cell, reward_value) or (None, -inf) if no goals available.
        """
        size = self.cfg.grid_size
        max_steps = max(0, self.cfg.goal_visibility_steps)
        best: Optional[Cell] = None
        best_val = float('-inf')

        def within_window(cell: Cell) -> bool:
            return manhattan(self.pos, cell) <= max_steps
        ...
        # Pre-clue: forward bias with side check secondary
        # Post-clue: pure probability, no directional preference
        if self.heading != (0, 0):
            fx = self.pos[0] + self.heading[0]
            fy = self.pos[1] + self.heading[1]
            forward = (fx, fy)
            if 0 <= fx < size and 0 <= fy < size and within_window(forward):
        ...
        # Search all cells in window for highest probability
        for cell in window_cells:
            if cell in self.know.visited:
                continue
            # Pre-clue: only consider immediate orthogonal neighbors (no diagonals)
            if not self.know.first_clue_seen and manhattan(self.pos, cell) != 1:
                continue
            idx = self._idx(cell[0], cell[1])
            val = prob_map[idx] * self.cfg.reward_factor
            if val > best_val:
                best_val = val
                best = cell

        # Fallback: nearest unvisited or least-visited cell in window
        if best is None:
            unknown = [cell for cell in window_cells if cell not in self.know.visited]
            if unknown:
                best = min(unknown, key=lambda c: manhattan(self.pos, c))
                idx = self._idx(best[0], best[1])
                best_val = prob_map[idx] * self.cfg.reward_factor
            else:
                revisit_candidates = [cell for cell in window_cells if cell != self.pos]
                if not revisit_candidates:
                    return None, float('-inf')
                best = min(
                    revisit_candidates,
                    key=lambda c: (self.know.visited.get(c, 0), manhattan(self.pos, c)),
                )
                idx = self._idx(best[0], best[1])
                best_val = prob_map[idx] * self.cfg.reward_factor
    ...
    def simple_energy_metrics(self, elapsed_ms: int) -> Dict[str, Any]:
        """Return basic runtime/message metrics for post-analysis."""
        compute_time_ms = elapsed_ms

        total_msgs_sent = (self.position_msgs_sent + self.visited_msgs_sent + self.clue_msgs_sent +
                          self.object_msgs_sent + self.intent_msgs_sent + self.goal_msgs_sent)
        total_msgs_received = (self.position_msgs_received + self.visited_msgs_received +
                              self.clue_msgs_received + self.object_msgs_received + self.intent_msgs_received + self.goal_msgs_received)
    ...
    def preview_goal(self, reserved_goals: Dict[str, Optional[Cell]]) -> None:
        if self.comm is None:
    ...
    def step_once(
        ...
        plan_peer_positions_raw, plan_peer_intents_raw, plan_peer_goals_raw = self._promote_perception()
        ...
        # Ensure current goal is still valid (not visited)
        if self.current_goal is not None:
            if self.current_goal in self.know.visited:
                ...
                reserved_goals.pop(self.rid, None)
                # C algorithm doesn't use goal coordination, so no clear needed
        ...
        # Set up peer data for planning using perceived (delayed) comm snapshots
        perceived_positions = dict(plan_peer_positions_raw)
        perceived_positions[self.rid] = self.pos
        plan_peer_positions = perceived_positions
        perceived_intents = dict(plan_peer_intents_raw)
        if self.rid in peer_intents:
            perceived_intents[self.rid] = peer_intents[self.rid]
        plan_peer_intents = perceived_intents
        # Goals are currently unused in Algorithm C, but we retain the snapshot for parity.
        _plan_peer_goals = plan_peer_goals_raw

        candidate_goal, candidate_reward = self.pick_goal(prob_map, reserved_goals, plan_peer_positions, plan_peer_intents)
        goal = candidate_goal
        EPS = 1e-9
        if self.current_goal is not None:
            if goal is None:
        ...
        if goal_changed:
            ...
            self.goal_completed_by_self_pending = False
            # C algorithm doesn't use goal coordination, so no publish needed

        reserved_goals[self.rid] = self.current_goal

        # Plan path so the viewer can render it (peer data already set up above)
    ...
    def _execute_pending_action(
        ...
        if kind == "move":
            ...
            if blocked:
                # Robot ID tie-breaking: if both heading to same goal, lower ID proceeds
                goal_conflict = (next_cell == self.current_goal)
                should_yield = True
                if goal_conflict and blocking_peer_id is not None:
                    # Lower robot ID gets priority
                    if self.rid < blocking_peer_id:
                        should_yield = False
                ...
                if should_yield:
                    ...
                    self._clear_pending_actions()
                    self.last_next_cell = None
                    # If this was a goal conflict, force goal replan
                    if goal_conflict:
                        self.current_goal = None
                        reserved_goals.pop(self.rid, None)
                    return False, "collision_yield"
                # else: lower ID robot proceeds despite blocking peer
            prev_steps = self.steps_taken
            self.steps_taken += 1
            self.pos = next_cell
            step_delta = self.steps_taken - prev_steps
            if self.know.first_clue_seen and (
                self.know.first_clue_step is None or self.steps_taken > self.know.first_clue_step
            ):
                self.steps_after_first_clue += step_delta
            self.path_history.append(self.pos)
            self.heading = heading
            if self.comm:
                self.comm.publish_position(self.pos)
            self.record_visit(self.pos)
            if self.comm:
                self.comm.publish_visited(self.pos)
            if self.pos in self.world.clue_cells and self.pos not in self.know.known_clues:
                self.record_clue(self.pos)
                if self.comm:
...
class EpisodeResult:
    ...
    discovered_clues: int  # Number of clues discovered
    ...
    collision_replans: Dict[str, int]  # Per-robot collision replans
    goal_replans: Dict[str, int]  # Per-robot goal replans
    collision_replans_total: int
    goal_replans_total: int
    revisits: int  # Individual revisits
    system_revisits: int  # System-wide revisits
    unique_cells: int  # Total unique cells visited by the team

    @property
    def replan_counts(self) -> Dict[str, int]:
        # Keep for backwards compatibility if needed
        return {rid: self.collision_replans.get(rid, 0) + self.goal_replans.get(rid, 0) for rid in set(self.collision_replans) | set(self.goal_replans)}
...
class MetricsPlotter:
    ...
    def update(self, metrics: EpisodeResult, cfg: Config, episode_index: Optional[int], tick: int) -> None:
        ...
        self.ax.clear()
        self.ax.axis("off")
        collision_total = metrics.collision_replans_total
        goal_total = metrics.goal_replans_total
...
def run_episode(cfg: Config, trial: Tuple[Cell, List[Cell]], episode_index: Optional[int] = None) -> EpisodeResult:
    ...
    for rid, pos, heading in start_states(cfg):
        local_know = Knowledge()
        rb = Robot(rid=rid, pos=pos, heading=heading, cfg=cfg, world=world, know=local_know)
        rb.path_history.append(pos)
        rb.record_visit(pos)
        if rb.pos in world.clue_cells and rb.pos not in rb.know.known_clues:
            rb.record_clue(rb.pos)
        if rb.pos == world.object_cell:
            found_at_spawn = True
        rb.comm = CommModule(rb, comm_bus)
        rb.comm.set_time(0.0)
        rb.comm.system_visits[pos] = rb.comm.system_visits.get(pos, 0) + 1
    ...
    if found_at_spawn:
        ...
        team_view = aggregate_team_knowledge(robots)
        return collect_episode_result(world, team_view, robots, steps_total, True)
    ...
    steps_total = sum(rb.steps_taken for rb in robots)
    team_view = aggregate_team_knowledge(robots)
    return collect_episode_result(world, team_view, robots, steps_total, found)
...
def run_viewer(cfg: Config) -> None:
    ...
    counter_file = "viewer_trial_counter_C_async.txt"
    ...
    class ViewerApp:
        ...
        def new_episode(self, reset_trial_index: bool = False, replay: bool = False) -> None:
            ...
            self.robots = []
            preview_reserved: Dict[str, Optional[Cell]] = {}
            for rid, pos, heading in start_states(self.cfg):
                local_know = Knowledge()
                rb = Robot(rid=rid, pos=pos, heading=heading, cfg=self.cfg, world=self.world, know=local_know)
                rb.path_history.append(pos)
                rb.record_visit(pos)
                ...
                preview_reserved[rb.rid] = None
            for rb in self.robots:
                rb.preview_goal(preview_reserved)
                rb.last_prob_map = rb.build_prob_map()
            self.initial_positions = {rb.rid: rb.pos for rb in self.robots}
            self.reserved_goals: Dict[str, Optional[Cell]] = {rb.rid: rb.current_goal for rb in self.robots}
            self.tick_count = 0
            # Deliver initial messages (e.g., spawn clues/positions)
            self.comm_bus.pump(0.0)
            self.steps_since_clue = 0
            if not self.found:
                self.found_by = None
        ...
        def advance_tick(self) -> None:
            ...
            # Update comm time for all robots
            current_time = float(self.tick_count)
            for rb in self.robots:
                if rb.comm:
                    rb.comm.set_time(current_time)

            # Pump messages from previous tick
            self.comm_bus.pump(current_time)
            ...
            for rb in sorted(self.robots, key=lambda r: r.rid):
                peer_positions = {r.rid: r.pos for r in self.robots}
                # Telemetry checkpoint: position and intent sharing happens here
                found_object, reason = rb.step_once(self.reserved_goals, peer_positions, peer_intents)
                self.reserved_goals[rb.rid] = rb.current_goal
        ...
        def update_team_knowledge(self) -> None:
            if not self.robots:
                ...
                self.know = aggregate_team_knowledge(self.robots)
...
def parse_args(argv=None) -> Config:
    base_cfg = Config()
    p = argparse.ArgumentParser(description="Pololu Search Simulator v3 async (C-00 Local Greedy with async scheduling)")
    ...
    p.add_argument("--goal-visibility-steps", type=int, default=base_cfg.goal_visibility_steps,
                   help="Max Manhattan distance (steps) considered when picking goals")
    ...
    p.add_argument("--async-tick-span", type=float, default=base_cfg.async_tick_span,
                   help="World time advanced per viewer tick (default: max(delay, mean)).")
    ...
    return Config(
        ...
        goal_visibility_steps=a.goal_visibility_steps,
        episodes=a.episodes,
        mode=a.mode,
        csv_path=a.csv_path,
