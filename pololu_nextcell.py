...
#   * After first clue: probabilistically chase rewards from prob_map rather
#     than planning long paths.
...
def distance_from_center(x):
    ...
    Used to penalize center-ward moves so robots tend to stay in their own
    regions without hard boundaries.
    """
    return abs(x - GRID_CENTER)

def centerward_step_cost(curr_x, next_x):
    """
    Penalize stepping toward the center *more than* turning to keep robots
    roughly in their starting regions and avoid clumping.
...
def choose_next_cell():
    """Select the next adjacent cell to move into.

    The choice is probabilistic based on local reward (prob_map) minus
    penalties for center-ward moves, revisiting cells, and peer intent.
    Returns the chosen (x, y) or None if no moves are available.
    """
    candidates = []
    weights = []
    cx, cy = pos[0], pos[1]
    for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
        ...
        if i_should_yield(nx, ny):
            continue
        w = prob_map[i] * REWARD_FACTOR
        w -= centerward_step_cost(cx, nx)
        if grid[i] == 2:
            w -= cfg.VISITED_STEP_PENALTY
        if w < 0:
            w = 0.0
    ...
    for move, w in zip(candidates, weights):
        if r <= w:
            return move
        r -= w
    return candidates[-1]
...
def search_loop():
    ...
    High-level mission loop:
      ...
      2) Choose next step probabilistically from adjacent cells
