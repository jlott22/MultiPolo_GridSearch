...
METRICS_LOG_FILE = "metrics-log-00B.txt"
...
def pick_goal():
    """Global goal selection with forward/side tie-breaks.

    Searches entire grid for highest probability cell.
    Pre-clue: forward preference as tie-breaker.
    Post-clue: pure probability ordering.
    """
    # Simple energy tracking - no function call counting needed
    best = None
    best_val = -1e30
    ...
    if heading != (0, 0) and 0 <= fx < GRID_SIZE and 0 <= fy < GRID_SIZE:
        i = idx(fx, fy)
    ...
    if best is None:
        ...
        if unknowns:
            ...
            best_idx = idx(best[0], best[1])
            best_val = prob_map[best_idx] * REWARD_FACTOR
        else:
            best = None
            best_val = -1e30
        ...
        while running and not found_object:
            ...
            try:
                ...
                if current_goal is not None:
                    current_idx = idx(current_goal[0], current_goal[1])
                    if grid[current_idx] != CELL_UNSEARCHED:
                        current_goal = None

                candidate_goal, candidate_reward = pick_goal()
                goal = candidate_goal

                current_goal_reward = -1e30
                if current_goal is not None:
                    current_idx = idx(current_goal[0], current_goal[1])
                    current_goal_reward = prob_map[current_idx] * REWARD_FACTOR
                ...
                if current_goal is not None:
                    ...
                    elif goal != current_goal and candidate_reward <= current_goal_reward + eps:
                        goal = current_goal
                        candidate_reward = current_goal_reward
