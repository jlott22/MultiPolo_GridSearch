...
#
"""
Pololu Search Simulator (v3) -- B-00 Forward-Priority Algorithm with Energy Tracking,
...
- Goal selection: Forward/side-biased pre-clue, then global argmax search. No goal reservation.
...
- Object and clue placements load from CSV/trial inputs when provided (default: Test-scenarios-B.csv) and otherwise come from the external generator.
...
Usage examples:
  python3 B-sim.py --mode batch --episodes 200 --robots 4 --grid 10 --csv out.csv
  python3 B-sim.py --mode batch --robots 2 --clues 3 --episodes 500 --trials-out trials.json
  python3 B-sim.py --mode view --robots 4 --grid 10 --viewer-fps 1 --show-truth --trials-in trials.json
...
def load_scenarios_csv(path: str) -> List[Dict[str, List[Cell]]]:
    ...
    with open(path, "r", newline="") as csvfile:
        # Skip commented/blank lines so DictReader sees the real header row.
        filtered_lines = (
        ...
        # Build case/whitespace-insensitive lookup for columns so we can accept headers like
        # "OBJECT_X" or "object x", which show up in some generator outputs.
        ...
        for row in reader:
            ...
            while True:
                ...
                # Some exported CSVs skip numbering after the first gap, so stop once either is missing.
...
class Robot:
    ...
    local_visits: Dict[Cell, int] = field(default_factory=dict, repr=False)
    ...
    def _execute_pending_action(
        ...
        if kind == "move":
            ...
            previous_team_visits = self.know.visited.get(self.pos, 0)
            if previous_team_visits > 0:
                self.know.system_revisits += 1

            previous_local_visits = self.local_visits.get(self.pos, 0)
            if previous_local_visits > 0:
                self.individual_revisits += 1
                self.know.individual_revisits += 1
            self.local_visits[self.pos] = previous_local_visits + 1

            self.know.visited[self.pos] = previous_team_visits + 1
...
def run_episode(cfg: Config, trial: Tuple[Cell, List[Cell]]) -> EpisodeResult:
    ...
    for rid, pos, heading in start_states(cfg):
        ...
        previous_team_visits = know.visited.get(pos, 0)
        if previous_team_visits > 0:
            know.system_revisits += 1
        rb.local_visits[pos] = rb.local_visits.get(pos, 0) + 1
        know.visited[pos] = previous_team_visits + 1
    ...
    class ViewerApp:
        ...
        def serialize_state(self, note: str) -> Dict[str, Any]:
            ...
            snapshot: Dict[str, Any] = {
                ...
                "knowledge": {
                    ...
                    "first_clue_step": self.know.first_clue_step,
                    "individual_revisits": self.know.individual_revisits,
                    "system_revisits": self.know.system_revisits,
                    "robot_positions_at_first_clue": {
                        rid: [pos[0], pos[1]] for rid, pos in self.know.robot_positions_at_first_clue.items()
                    },
                },
        ...
        def new_episode(self, reset_trial_index: bool = False, replay: bool = False) -> None:
            ...
            for rid, pos, heading in start_states(self.cfg):
                ...
                rb.path_history.append(pos)
                previous_team_visits = self.know.visited.get(pos, 0)
                if previous_team_visits > 0:
                    self.know.system_revisits += 1
                rb.local_visits[pos] = rb.local_visits.get(pos, 0) + 1
                self.know.visited[pos] = previous_team_visits + 1
            ...
            if self.know.first_clue_seen and self.know.first_clue_step is None:
                self.know.first_clue_step = 0
                self.know.robot_positions_at_first_clue = {rb.rid: rb.pos for rb in self.robots}
        ...
        def advance_tick(self) -> None:
            ...
            for rb in sorted(self.robots, key=lambda r: r.rid):
                ...
                if self.know.first_clue_seen and self.know.first_clue_step is None:
                    self.know.first_clue_step = sum(r.steps_taken for r in self.robots)
                    self.know.robot_positions_at_first_clue = {r.rid: r.pos for r in self.robots}
...
def parse_args(argv=None) -> Config:
    base_cfg = Config()
    p = argparse.ArgumentParser(description="Pololu Search Simulator v3 (B-00 Forward-Priority)")
