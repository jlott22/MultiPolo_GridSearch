...
goal_forced_by_peer = False     # set when another robot invalidates our goal
...
# Simple energy tracking - message counters only (intent stays zero: disabled)
...
def record_intersection(x, y):
    ...
    global intersection_count, system_revisits, system_visits
...
first_clue_seen = False                # metrics only; behavior ignores this flag
move_forward_flag = False

# Intent data shared by peers (E algorithm never publishes intents itself)
peer_intent = {}  # peer_id -> (x, y) reservation
peer_pos = {}     # peer_id -> (x, y) last reported position
peer_goal = {}    # peer_id -> (x, y) goal reservation (we do not broadcast ours)
current_goal = None  # our reserved goal cell (local only, no broadcast)
...
# Always-on cost shaping keeps the serpentine sweep from collapsing inward.
# It must exceed the TURN_COST (1.0).
TURN_COST = 1.0
CENTER_STEP = 0.4
...
def publish_intent(x, y):
    """E algorithm: intent broadcasts are disabled."""
    return


def publish_goal(x, y):
    """E algorithm: goal broadcasts are disabled."""
    return
...
def handle_msg(line):
    ...
    """
    global peer_intent, peer_pos, peer_goal, current_goal, first_clue_seen, object_location, start_signal, found_object, FIRST_CLUE_TIME_MS, goal_replan_count, goal_forced_by_peer
...
def update_prob_map():
    """
    Recompute prob_map.
    E algorithm: keep a uniform distribution across every unvisited cell,
    regardless of any clues that have been discovered.
    """
...
def centerward_step_cost(curr_x, curr_y, next_x, next_y):
    """Always penalize steps that move inward toward the center on either axis."""
    cost = 0.0
    if next_x != curr_x:
...
def a_star(start, goal):
    ...
    # E algorithm: keep full turn cost regardless of clues.
...
def search_loop():
    ...
    """
    global first_clue_seen, move_forward_flag, start_signal, METRIC_START_TIME_MS, pos, yield_count, path_replan_count, goal_replan_count, FIRST_CLUE_TIME_MS, current_goal, goal_forced_by_peer, system_clues_found, FIRST_CLUE_POSITION, system_visits, busy_ms, mem_free_min
    ...
    try:
        ...
        while running and not found_object:
            ...
            try:
                ...
                goal = pick_goal()
                if goal is None:
                    goal_forced_by_peer = False
                    break

                if goal != prev_goal:
                    if goal_forced_by_peer:
                        goal_replan_count += 1
                    current_goal = goal
                    goal_forced_by_peer = False
                ...
                # Process any pending inbound messages before stepping
                uart_service()
