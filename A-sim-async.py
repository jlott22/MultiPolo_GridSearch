...
#
"""
Pololu Search Simulator (v3 async) -- A-00-matching behavior with asynchronous robot clocks, fixed placements per episode,
batch + viewer.
...
Author: James Lott
TODO: fix 00 code to match counting start cell in unique cells, fix dist from first clue metric- should be manhattan distance to object at the end.
"""
# Viewer controls quick reference:
#  Esc/q: quit | p: pause/resume | . step while paused | auto-start delay configurable
...
#  6. CSV results auto-save to: sim_results_A_async.csv (in current directory)
...
VIEWER_AUTOSTART_DELAY_SECONDS = 7  # Delay before viewer begins playback
ENABLE_METRICS_VIEW = False  # Disable matplotlib metrics window by default
...
class Config:
    ...
    episodes: int = 50
    mode: str = "view"
    csv_path: str = 'sim_results_A3.csv' #'sim_results_A1.csv'
    viewer_fps: int = 2
    cell_px: int = 800
    show_truth_in_viewer: bool = True
    trials_in: Optional[str] = None
    trials_out: Optional[str] = None
    scenarios_csv: Optional[str] = "trials_presentation.csv"
    start_trial: int = 0  # Which trial to start with (0, 1, 2, etc). Auto-increments after each viewer run.
    async_step_mean: float = 1.65
    async_step_jitter: float = 0.005
    async_min_delay: float = 1.555
    ...
    comm_delay_s: float = 0.04  # Communication delay between robots
# ------------------------
...
def aggregate_team_knowledge(robots: List['Robot']) -> Knowledge:
    """Combine per-robot knowledge into a single team-level view (viewer-only)."""
    agg = Knowledge()
...
class Robot:
    ...
    def sync_from_comm(self) -> None:
        """
        Pull delayed team updates from the comm module into this robot's local knowledge.
        Mirrors how the firmware applies peer UART messages: only information that has
        actually been delivered on the bus is reflected here.
        """
        if self.comm is None:
            return

        # Integrate visited counts learned from team traffic
        for cell, count in self.comm.system_visits.items():
            existing = self.know.visited.get(cell, 0)
            if count > existing:
                self.know.visited[cell] = count

        # Integrate discovered clues (order preserved by arrival)
        new_clue = False
    ...
    def _capture_perception(self) -> None:
        """
        Snapshot peer state for use the next time we plan. This mirrors firmware behaviour
        where new UART traffic is incorporated between moves, not mid-planning.
        """
        if self.comm is None:
    ...
    def _promote_perception(self) -> Tuple[Dict[str, Cell], Dict[str, Optional[Cell]], Dict[str, Optional[Cell]]]:
        """
        Provide the frozen peer view to planning, only refreshing when a new snapshot
        has been captured at the end of the previous move.
        """
    ...
    def pick_goal(
        self,
        prob_map: List[float],
        perceived_goals: Dict[str, Optional[Cell]],
        ...
        reserved_by_peers = {
            cell
            for rid, cell in perceived_goals.items()
            if rid != self.rid and cell is not None
        }
    ...
    def preview_goal(self, reserved_goals: Dict[str, Cell]) -> None:
        if self.comm is None:
            raise RuntimeError("preview_goal requires an active CommModule for goal coordination.")
        self.sync_from_comm()
        prob_map = self.build_prob_map()
        perceived_goals = self.comm.snapshot_peer_goals()
        goal, _ = self.pick_goal(prob_map, perceived_goals)
    ...
    def step_once(self, reserved_goals: Dict[str, Cell],
        ...
        if self.pending_actions:
            result = self._execute_pending_action(reserved_goals, peer_positions, peer_intents)
            return result

        # Use the frozen peer view captured when the previous move finished.
        plan_peer_positions, plan_peer_intents, plan_peer_goals = self._promote_perception()
        # Build prob map for decision making; keep a copy for visualization
        prob_map = self.build_prob_map()
        self.last_prob_map = prob_map[:]
        self.last_event = None
        if self.comm is None:
            raise RuntimeError("Robot.step_once requires an active CommModule for goal coordination.")
        ...
        candidate_goal, candidate_reward = self.pick_goal(
            prob_map,
            plan_peer_goals,
            plan_peer_positions,
            plan_peer_intents,
        ...
        if goal_changed:
            ...
            self.goal_completed_by_self_pending = False
            if self.comm and self.current_goal is not None:
                self.comm.publish_goal(self.current_goal)
    ...
    def _execute_pending_action(
        ...
        ground_peer_intents = {pid: cell for pid, cell in peer_intents.items() if pid != self.rid}
        ...
        if kind == "move":
            ...
            # Real robots only know peer state from delayed UART messages; freeze that view for this move.
            blocked = False
            if self.comm:
                ...
                perceived_intents = ground_peer_intents
                perceived_positions = ground_peer_positions
            ...
            # Prepare perception snapshot for the next planning cycle.
            self._capture_perception()
            self._clear_pending_actions()
            return False, None
...
def collect_episode_result(world: World, know: Knowledge, robots: List['Robot'], steps_total: int, found: bool) -> EpisodeResult:
    ...
    # Calculate steps before/after first clue using system totals
...
class MetricsPlotter:
    ...
    def update(self, metrics: EpisodeResult, cfg: Config, episode_index: Optional[int], tick: int) -> None:
        ...
        base_lines = [
            ...
            f"collision_total={metrics.collision_replans_total} goal_total={metrics.goal_replans_total}",
        ]
        ...
        # Format shortened CSV row for display
        clue_locs_short = f"{len(metrics.clue_cells)}clues"
        discovered_short = f"{len(metrics.discovered_clue_locations)}found"
        csv_values = [
            ...
            cfg.clue_count,
            metrics.steps_total,
            f"({metrics.object_cell[0]},{metrics.object_cell[1]})",
            clue_locs_short,
            int(metrics.found),
            discovered_short,
...
def run_viewer(cfg: Config) -> None:
    ...
    counter_file = "viewer_trial_counter_A_async.txt"
    ...
    class ViewerApp:
        def __init__(self, cfg: Config, trials: List[Dict[str, List[Cell]]]):
            ...
            if ENABLE_METRICS_VIEW and plt is not None:
                try:
            ...
            self.paused = True
            self.autostart_delay_remaining: Optional[float] = max(0.0, VIEWER_AUTOSTART_DELAY_SECONDS)
            self.cell_px = 40
            self.margin = 40
            self.panel_w = 260
            self.info_h = 140
        ...
        def new_episode(self, reset_trial_index: bool = False, replay: bool = False) -> None:
            ...
            for rid, pos, heading in start_states(self.cfg):
                ...
                if rb.current_goal is not None:
                    rb.comm.publish_goal(rb.current_goal)
                rb.last_prob_map = rb.build_prob_map()
            ...
            self.follow_history = True
            self.paused = True
            self.autostart_delay_remaining = max(0.0, VIEWER_AUTOSTART_DELAY_SECONDS)
        ...
        def handle_key(self, event: Any) -> None:
            ...
            if key in (pygame.K_ESCAPE, pygame.K_q):
                ...
                return
            if key == pygame.K_p:
                self.autostart_delay_remaining = None
                self.paused = not self.paused
                if not self.paused:
                    step_interval = 1.0 / max(self.viewer_fps, self.min_fps)
                    self.viewer_accumulator = max(self.viewer_accumulator, step_interval)
                return
        ...
        def run(self) -> None:
            ...
            while running:
                ...
                if not running:
                    break
                if self.autostart_delay_remaining is not None and not self.found:
                    self.autostart_delay_remaining = max(0.0, self.autostart_delay_remaining - dt)
                    if self.autostart_delay_remaining <= 0.0:
                        self.autostart_delay_remaining = None
                        self.paused = False
                        step_interval = 1.0 / max(self.viewer_fps, self.min_fps)
                        self.viewer_accumulator = max(self.viewer_accumulator, step_interval)
...
def parse_args(argv=None) -> Config:
    base_cfg = Config()
    p = argparse.ArgumentParser(description="Pololu Search Simulator v3 async (A-00-matching core)")
