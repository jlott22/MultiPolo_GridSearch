...
#
"""
Pololu Search Simulator (v3) -- C-00 Local Greedy Algorithm with Energy Tracking,
fixed placements per episode, batch + viewer.

Key behaviors (C-00 specific):
...
- Energy tracking: Motor-time accumulator assumes 800mA draw during movement/turns.
  CPU usage is estimated from provided elapsed time when metrics are requested; message counters remain placeholders.
...
Usage examples:
  python3 C-sim.py --mode batch --episodes 200 --robots 4 --grid 10 --csv out.csv
  python3 C-sim.py --mode batch --robots 2 --clues 3 --episodes 500 --trials-out trials.json
  python3 C-sim.py --mode view --robots 4 --grid 10 --viewer-fps 1 --show-truth --trials-in trials.json
...
#  6. CSV results auto-save to: sim_results_B.csv (in current directory)
...
class Config:
    ...
    episodes: int = 15
    mode: str = "batch"
    csv_path: str = "sim_results_C3.csv"
    viewer_fps: int = 4
    cell_px: int = 15
    show_truth_in_viewer: bool = True
    trials_in: Optional[str] = None
    trials_out: Optional[str] = None
    scenarios_csv: Optional[str] = "trials_conference1+r^2.csv" # "sim_results_C.csv"
...
class Knowledge:
    ...
    individual_revisits: int = 0  # Repeat visits by the robot that revisits
    system_revisits: int = 0      # Repeat visits to cells already seen by any teammate
    first_clue_step: Optional[int] = None  # Steps taken by the discovering robot when first clue found
...
def load_scenarios_csv(path: str) -> List[Dict[str, List[Cell]]]:
    ...
    with open(path, "r", newline="") as csvfile:
        filtered_lines = (
            line for line in csvfile if line.strip() and not line.lstrip().startswith("#")
        )
        reader = csv.DictReader(filtered_lines)

        if not reader.fieldnames:
            raise ValueError(f"CSV '{path}' does not contain a header row.")
        ...
        for name in reader.fieldnames:
            ...
            if norm in norm_to_actual:
                continue
            norm_to_actual[norm] = name

        def resolve_column(expected: str) -> str:
            key = expected.lower().replace(" ", "_")
            if key not in norm_to_actual:
                raise ValueError(
                    f"CSV '{path}' missing expected column '{expected}'. "
                    f"Available columns: {', '.join(reader.fieldnames)}"
                )
            return norm_to_actual[key]

        obj_x_col = resolve_column("object_x")
        obj_y_col = resolve_column("object_y")

        for row in reader:
            if not row.get(obj_x_col) or not row.get(obj_y_col):
                continue

            try:
                obj = (int(row[obj_x_col]), int(row[obj_y_col]))
            except ValueError as exc:
                raise ValueError(f"Invalid object coords in '{path}': {row}") from exc

            clues: List[Cell] = []
            clue_num = 1
            while True:
                clue_x_norm = f"clue{clue_num}_x"
                clue_y_norm = f"clue{clue_num}_y"
                clue_x_key = norm_to_actual.get(clue_x_norm)
                clue_y_key = norm_to_actual.get(clue_y_norm)
                if not clue_x_key or not clue_y_key:
                    break
                clue_x_val = row.get(clue_x_key)
                clue_y_val = row.get(clue_y_key)
                if not clue_x_val or not clue_y_val:
                    break
                try:
                    clues.append((int(clue_x_val), int(clue_y_val)))
                except ValueError as exc:
                    raise ValueError(
                        f"Invalid clue coords for clue{clue_num} in '{path}': {row}"
                    ) from exc
                clue_num += 1
...
class Robot:
    ...
    # Simple energy tracking - message counters
    ...
    def simple_energy_metrics(self, elapsed_ms: int) -> Dict[str, Any]:
        """Calculate simple energy metrics - robust and accurate."""
        # Compute time = everything except motor time
        compute_time_ms = elapsed_ms - self.motor_time_ms
        ...
        motor_energy_estimate = self.motor_time_ms * MOTOR_POWER_MA
        compute_energy_estimate = compute_time_ms * CPU_POWER_MA
        total_energy_estimate = motor_energy_estimate + compute_energy_estimate

        return {
            'motor_time_ms': self.motor_time_ms,
            'compute_time_ms': compute_time_ms,
            'msgs_sent': total_msgs_sent,
            'msgs_received': total_msgs_received,
            'motor_energy_ma_ms': motor_energy_estimate,
            'compute_energy_ma_ms': compute_energy_estimate,
            'total_energy_ma_ms': total_energy_estimate
        }

    def preview_goal(self) -> None:
        # Simple energy tracking - no function call counting needed
        prob_map = self.build_prob_map()
        goal, _ = self.pick_goal(prob_map)
        self.current_goal = goal

    def step_once(self, peer_positions: Dict[str, Cell],
        ...
        if self.pending_actions:
            return self._execute_pending_action(peer_positions, peer_intents)
        ...
        if goal_changed:
            ...
            # Count goal replans only after first clue is found and after initial goal assignment
            if (self.know.first_clue_seen and previous_goal is not None and
                    not self.path_replan_pending and not self.goal_completed_by_self_pending):
                self.goal_replans += 1
    ...
    def _execute_pending_action(
        ...
        if kind == "move":
            ...
            if blocked:
                ...
                self.last_next_cell = None
                return False, "stuck"
            revisited_by_self = next_cell in self.path_history
            team_visit_count = self.know.visited.get(next_cell, 0)
            ...
            # Simple energy tracking - motor time for movement
            self.motor_time_ms += 100  # Assume 100ms per step
            if revisited_by_self:
                self.individual_revisits += 1
                self.know.individual_revisits += 1
            if team_visit_count > 0:
                self.know.system_revisits += 1
            self.know.visited[self.pos] = team_visit_count + 1
            if self.pos in self.world.clue_cells and self.pos not in self.know.known_clues:
                ...
                # Simple energy tracking - clue messages would be sent here
                self.last_event = "clue_found"
            if self.pos == self.world.object_cell:
                self.last_step_reason = "found_object"
                # Simple energy tracking - object found messages would be sent here
...
class MetricsPlotter:
    ...
    def update(self, metrics: EpisodeResult, cfg: Config, episode_index: Optional[int], tick: int) -> None:
        ...
        base_lines = [
            ...
            f"unique_cells={metrics.unique_cells}",
...
def run_episode(cfg: Config, trial: Tuple[Cell, List[Cell]]) -> EpisodeResult:
    ...
    for rid, pos, heading in start_states(cfg):
        ...
        if rb.pos in world.clue_cells and rb.pos not in know.known_clues:
            ...
            if know.first_clue_step is None:
                know.first_clue_step = rb.steps_taken
            know.robot_positions_at_first_clue.setdefault(rb.rid, rb.pos)
    ...
    while not found:
        ...
        for rb in sorted(robots, key=lambda r: r.rid):
            ...
            f, _ = rb.step_once(peer_positions, peer_intents)
            peer_positions[rb.rid] = rb.pos
            if know.first_clue_seen and len(know.robot_positions_at_first_clue) < len(robots):
                for other in robots:
                    know.robot_positions_at_first_clue.setdefault(other.rid, other.pos)
            if f:
                found = True
                break
...
def run_viewer(cfg: Config) -> None:
    ...
    counter_file = "viewer_trial_counter_B.txt"
    ...
    class ViewerApp:
        ...
        def new_episode(self, reset_trial_index: bool = False, replay: bool = False) -> None:
            ...
            for rid, pos, heading in start_states(self.cfg):
                ...
                if rb.pos in self.world.clue_cells and rb.pos not in self.know.known_clues:
                    ...
                    self.know.first_clue_seen = True
                    if self.know.first_clue_step is None:
                        self.know.first_clue_step = rb.steps_taken
                    self.know.robot_positions_at_first_clue.setdefault(rb.rid, rb.pos)
        ...
        def advance_tick(self) -> None:
            ...
            for rb in sorted(self.robots, key=lambda r: r.rid):
                ...
                # Simple energy tracking - message exchange point (position and intent sharing)
                found_object, reason = rb.step_once(peer_positions, peer_intents)
                ...
                if found_object:
                    ...
                    break
                if self.know.first_clue_seen and len(self.know.robot_positions_at_first_clue) < len(self.robots):
                    for other in self.robots:
                        self.know.robot_positions_at_first_clue.setdefault(other.rid, other.pos)
        ...
        def export_episode_summary_csv(self) -> None:
            ...
            with open(path, "w", newline="") as fh:
                ...
                row = batch_csv_row(metrics, self.cfg, self.active_trial_index if self.active_trial_index is not None else 0)
                writer.writerow(row)
            self.log_event("export", f"Saved {filename}")
        ...
        def display_episode_results(self) -> None:
            ...
            row_values = batch_csv_row(metrics, self.cfg, self.active_trial_index if self.active_trial_index is not None else 0)
            ...
            print(f"clue_locations: {clue_locs_str or '-'}")
            print(f"discovered_clue_locations: {discovered_locs_str or '-'}")
            print(f"robot_positions_at_first_clue: {robot_pos_str or '-'}")
...
def parse_args(argv=None) -> Config:
    base_cfg = Config()
    p = argparse.ArgumentParser(description="Pololu Search Simulator v3 (C-00 Local Greedy)")
